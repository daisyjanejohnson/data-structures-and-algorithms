...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(person => {
    ...
    let $appendNewPerson = $(`<section>${template}</section`);
    $appendNewPerson.find('h2').text(person.name);
    $appendNewPerson.find('h3').text(person.height);
    $appendNewPerson.find('p').text(person.eye_color);
    $('main').append($appendNewPerson);
  })
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    // cut off the numbers
    let firstIndex = ingredient.indexOf(' ');
    let secondStr = ingredient.split(firstIndex + 1);
    //cut off the units
    let secondIndex = secondStr.indexOf(' ');
    let finalStr = secondStr.split(secondIndex + 1);
