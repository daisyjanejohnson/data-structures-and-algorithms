...
const oddValues = (arr) => {
 let oddArr = arr.filter(value => value % 2 === 1);
 return oddArr;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const filterStringsWithVowels = (arr) => {
  const hasVowels = arr.filter(word => word.match(/[aeiou]/g));
  return hasVowels;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(num => minBaseStat < num.baseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(num => minBaseStat.name < num.baseStat.name);
};
